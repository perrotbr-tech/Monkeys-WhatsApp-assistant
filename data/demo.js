...
const monkeysRetencion = crearDatosRetencion();
const somaRetencion = crearDatosRetencion(undefined, { tenantId: 'soma' });
...
const somaSlice = {
  ...
  classes: clasesSoma(),
  plans: [
    { id: 'mensual', tenantId: 'soma', nombre: 'Plan Mensual', precio: '$69.990' },
    { id: 'trimestral', tenantId: 'soma', nombre: 'Plan Trimestral', precio: '$189.990' },
    { id: 'anual', tenantId: 'soma', nombre: 'Plan Anual', precio: '$649.990' },
    { id: 'kids', tenantId: 'soma', nombre: 'Functional Kids', precio: '$49.990' },
  ],
  bookings: [],
  leads: [
...
export const demo = monkeysSlice;

export function clonar(obj) {
  return JSON.parse(JSON.stringify(obj));
}

export function clonarDemo(tenantId = 'monkeys') {
  if (tenantId === 'soma') return clonar(somaSlice);
  return clonar(monkeysSlice);
}

export function clonarMundo() {
  return {
    tenants: listarTenants(),
    byTenant: {
      monkeys: clonar(monkeysSlice),
      soma: clonar(somaSlice),
    },
  };
}

export { monkeysSlice, somaSlice };
